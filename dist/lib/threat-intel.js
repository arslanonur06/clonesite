...
// URLVoid integration
export async function checkUrlVoid(domain) {
    ...
    try {
        ...
        return {
            engines,
            detections: detections.map((d) => d.replace(/<[^>]*>/g, ''))
        };
    }
    catch (error) {
...
export async function checkShodan(domain) {
    ...
    try {
        ...
        return {
            ports: data.ports || [],
            services: data.data?.map((service) => service.product || service.transport) || [],
...
// Passive DNS lookup
export async function checkPassiveDns(domain) {
    try {
...
export async function getThreatIntelligence(domain) {
...
export async function batchThreatIntelligence(domains) {
    const pLimit = (await import('p-limit')).default;
    const limit = pLimit(5); // Limit concurrent requests to avoid rate limiting
    return Promise.all(domains.map(domain => limit(() => getThreatIntelligence(domain))));
}
// Generate threat intelligence report
export async function generateThreatReport(intelligence, outputPath) {
    const report = {
