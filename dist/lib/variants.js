...
function splitDomain(domain) {
    ...
    if (parts.length < 2)
        return { label: domain, tld: '' };
    const tld = '.' + parts.slice(1).join('.');
    return { label: parts[0], tld };
}
export function generateDomainVariants(originalDomain, options) {
    ...
    function add(labelVariant, reason) {
    ...
    for (let i = 0; i < label.length - 1; i++) {
        const variant = label.slice(0, i) + label[i + 1] + label[i] + label.slice(i + 2);
