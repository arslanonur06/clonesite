...
class AhrefsIntegration {
     ...
     */
    async getPaidKeywords(domain, limit = 100) {
        try {
            const response = await this.makeRequest('/', {
                target: domain,
                mode: 'paid_keywords',
    ...
    calculateRiskLevel(domainInfo, backlinks) {
    ...
    generateRiskIndicators(domainInfo, backlinks) {
        const indicators = [];
...
export function createAhrefsIntegration() {
