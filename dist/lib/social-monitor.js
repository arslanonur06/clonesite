...
export async function monitorTwitter(brand, apiKey) {
...
export async function monitorTelegram(brand) {
    ...
    try {
        ...
        const messages = await page.$$eval('.tgme_widget_message', (elements) => elements.map(el => ({
...
export async function monitorEmailSecurity(domain) {
    try {
        // Check DMARC record
        const dmarcResponse = await axios.get(`https://dns.google/resolve?name=_dmarc.${domain}&type=TXT`);
        const hasDmarc = dmarcResponse.data.Answer?.some((record) => record.data.includes('v=DMARC1')) || false;
        // Check SPF record
        const spfResponse = await axios.get(`https://dns.google/resolve?name=${domain}&type=TXT`);
        const hasSpf = spfResponse.data.Answer?.some((record) => record.data.includes('v=spf1')) || false;
...
export async function runSocialMonitoring(brand) {
