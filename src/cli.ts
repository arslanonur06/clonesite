...
import { compareLogos } from './lib/ai-visual.js';
...
program
  ...
  .action(async (domainArg: string, options: { concurrency: string; extensions?: string }) => {
    ...
    for (const r of found) {
      ...
      console.log(
        `${chalk.bold(r.domain)} ${chalk.gray(`(${reason})`)} -> ` +
...
program
  ...
  .action(async (domain: string, baseUrl: string, opts: { out: string; db: string; limit: string }) => {
    ...
    for (const v of slice) {
      ...
      try {
        ...
        console.error(chalk.yellow(`Monitor failed for ${v.domain}: ${(e as Error).message}`));
      }
    }
  });
...
program
  ...
  .action(async (domain: string, baseUrl: string, opts: { out: string; db: string; hour: string }) => {
    await fs.mkdir(opts.out, { recursive: true });
...
program
  ...
  .action(async (baseUrl: string, brand: string, opts: { sequence?: string; range: string; ctDays: string; logo?: string; out: string }) => {
    ...
    try {
      ...
      console.error(chalk.yellow(`Social monitoring failed: ${(e as Error).message}`));
    }
    ...
    try {
      ...
      console.error(chalk.yellow(`CT log search failed: ${(e as Error).message}`));
    }
    ...
    const findings = [] as import('./lib/report.js').CloneFinding[];
    const pLimit = (await import('p-limit')).default;
    const limit = pLimit(3);
    
    await Promise.all(registered.slice(0, 50).map((r) => limit(async () => { // Limit to 50 for performance
      ...
      try {
        ...
        console.error(chalk.yellow(`✗ Skip ${r.domain}: ${(e as Error).message.slice(0, 80)}`));
      }
    })));
...
program
  ...
  .action(async (brand: string, opts: { logo?: string; out: string }) => {
...
program
  ...
  .action(async (brand: string, opts: { out: string }) => {
...
program
  ...
  .action(async (domains: string[], opts: { out: string }) => {
...
program
  ...
  .action(async (brand: string, domains: string[] = [], opts: { artifacts: string; out: string }) => {
...
program
  ...
  .action(async (baseUrl: string, url: string, options: { out: string; threshold: string }) => {
