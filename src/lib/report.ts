...
function renderHtml(title: string, baseUrl: string, findings: CloneFinding[]): string {
  const rows = findings
    .map((f) => `
      <tr>
        <td>${f.domain}</td>
...
export async function writePdfReport(title: string, baseUrl: string, findings: CloneFinding[], outPdfPath: string): Promise<void> {
