...
export type BonusOffer = {
  title: string;
  bonusType: 'welcome' | 'deposit' | 'freespin' | 'cashback' | 'generic';
  percentage?: number;
  maxAmount?: number;
  minDeposit?: number;
  wagering?: number;
...
export type AnalysisLanguage = 'auto' | 'tr' | 'en';
...
const BONUS_LINE_PATTERN_TR =
  /(hoş\s*geldin|bonus|promosyon|freespin|free\s*spin|yatırım|çevrim|cashback|kayıp\s*iadesi)/i;
const BONUS_LINE_PATTERN_EN =
  /(welcome|bonus|promotion|freespin|free\s*spin|deposit|wager|wagering|cashback|terms)/i;
...
function parseWagering(text: string): number | undefined {
  const match = text.match(/(\d{1,3})\s*x\s*(çevrim|wager|wagering|çrim|çevirim)?/i);
  if (!match) return undefined;
  return Number(match[1]);
}

function parseFreeSpins(text: string): number | undefined {
  const match = text.match(/(\d{1,4})\s*(adet\s*)?(freespin|free\s*spin)/i);
  if (!match) return undefined;
  return Number(match[1]);
}

function parseAmountNearKeywords(text: string, keywordRegex: RegExp): number | undefined {
  const keywordMatch = text.match(keywordRegex);
  if (!keywordMatch || keywordMatch.index === undefined) return undefined;
...
function classifyBonusType(text: string): BonusOffer['bonusType'] {
  const lower = text.toLowerCase();
  if (lower.includes('hoş geldin')) return 'welcome';
  if (lower.includes('freespin') || lower.includes('free spin')) return 'freespin';
  if (lower.includes('cashback') || lower.includes('kayıp iadesi')) return 'cashback';
  if (lower.includes('yatırım')) return 'deposit';
  return 'generic';
}

function extractCandidateLines($: cheerio.CheerioAPI, preferredLanguage: AnalysisLanguage): string[] {
  const pattern =
  ...
  $('h1, h2, h3, h4, p, li, span, div').each((_, el) => {
    ...
    if (!text || text.length < 20 || text.length > 320) return;
...
function buildOffers(lines: string[]): BonusOffer[] {
  ...
  const deduped = new Map<string, BonusOffer>();
...
function buildSummary(offers: BonusOffer[]) {
...
function detectLanguage(lines: string[], preferredLanguage: AnalysisLanguage): 'tr' | 'en' | 'unknown' {
  if (preferredLanguage === 'tr') return 'tr';
  if (preferredLanguage === 'en') return 'en';
  const text = lines.join(' ').toLowerCase();
  const score = TURKISH_HINTS.reduce((sum, token) => sum + (text.includes(token) ? 1 : 0), 0);
  if (score >= 2) return 'tr';
  if (/(welcome|promotion|wagering|deposit|terms|bonus)/i.test(text)) return 'en';
  return 'unknown';
}

export async function analyzeBonusPage(
  url: string,
  name?: string,
  preferredLanguage: AnalysisLanguage = 'auto'
): Promise<BonusProfile> {
  const errors: string[] = [];
  let html = '';
  ...
  try {
    ...
    errors.push(`fetch_failed: ${(error as Error).message}`);
  }
...
export function createSideBySideComparison(profiles: BonusProfile[]) {
