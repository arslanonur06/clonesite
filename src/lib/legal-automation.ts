...
export type TakedownRequest = {
  ...
  abuseContact: string;
  evidenceUrls: string[];
  requestType: 'phishing' | 'trademark' | 'copyright' | 'malware';
  status: 'pending' | 'sent' | 'acknowledged' | 'resolved' | 'rejected';
  createdAt: string;
  updatedAt: string;
};

export type LegalEvidence = {
  type: 'screenshot' | 'whois' | 'dns' | 'similarity_report';
  filePath: string;
  description: string;
  timestamp: string;
};
...
export function generateTakedownTemplate(request: TakedownRequest, brand: string): string {
  const templates = {
    phishing: `
...
export async function collectEvidence(domain: string, artifactPaths: string[]): Promise<LegalEvidence[]> {
  ...
  for (const artifactPath of artifactPaths) {
    try {
      ...
      let type: LegalEvidence['type'] = 'screenshot';
      let description = '';
...
export async function sendTakedownRequest(request: TakedownRequest, brand: string): Promise<boolean> {
  try {
...
export async function generateLegalReport(requests: TakedownRequest[], outputPath: string): Promise<void> {
  const report = {
...
export async function runLegalAutomation(
  domains: string[],
  brand: string,
  artifactDir: string
): Promise<TakedownRequest[]> {
