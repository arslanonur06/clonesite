...
export async function generateIGamingPdfReport(reportData: any, outPdfPath: string): Promise<string> {
...
function generateIGamingReportHTML(data: any): string {
      ...
      <div class="container">
        ...
        <div class="section">
          ...
          <div class="risk-matrix">
            <div class="risk-item">
              <strong>Domain Threats:</strong> ${(data.domains || []).filter((d: any) => d.riskLevel === 'high').length} High Risk
            </div>
            <div class="risk-item">
              <strong>Mobile Threats:</strong> ${(data.mobile || []).filter((m: any) => m.riskLevel === 'high').length} High Risk Apps
            </div>
            <div class="risk-item">
              <strong>Crypto Threats:</strong> ${(data.crypto || []).filter((c: any) => c.riskLevel === 'high').length} High Risk
            </div>
            <div class="risk-item">
        ...
        <div class="section">
          ...
          <p><strong>Total Domains Found:</strong> ${data.domains.length} | <strong>High Risk:</strong> ${data.domains.filter((d: any) => d.riskLevel === 'high').length}</p>
          <table>
            <thead>
              <tr>
                <th>Domain</th>
            ...
            <tbody>
              ${data.domains.slice(0, 50).map((domain: any) => `
                <tr>
