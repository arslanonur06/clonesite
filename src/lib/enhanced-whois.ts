...
export interface EnhancedWhoisData {
  domain: string;
...
class EnhancedWhoisChecker {
  ...
  async checkDomain(domain: string): Promise<EnhancedWhoisData> {
    try {
  ...
  private async getWhoisData(domain: string): Promise<Partial<EnhancedWhoisData>> {
    try {
      ...
      const data: Partial<EnhancedWhoisData> = {
  ...
  private parseContactInfo(contact: any): EnhancedWhoisData['adminContact'] {
  ...
  private analyzeWhoisRisk(whoisData: Partial<EnhancedWhoisData>): string[] {
    ...
    if (whoisData.nameServers) {
      ...
      const hasSuspiciousNs = whoisData.nameServers.some(ns =>
    ...
    if (whoisData.status) {
      ...
      const hasSuspiciousStatus = whoisData.status.some(status =>
        suspiciousStatuses.some(s => status.toLowerCase().includes(s))
      );
  ...
  private calculateRiskLevel(indicators: string[]): 'low' | 'medium' | 'high' {
    ...
    const hasHighRisk = indicators.some(indicator =>
      highRiskKeywords.some(keyword => indicator.includes(keyword))
    );

    const hasMediumRisk = indicators.some(indicator =>
      mediumRiskKeywords.some(keyword => indicator.includes(keyword))
    );
  ...
  async checkDomains(domains: string[]): Promise<EnhancedWhoisData[]> {
    return Promise.all(domains.map(domain => this.limit(() => this.checkDomain(domain))));
  }
}

export function createEnhancedWhoisChecker(concurrency = 20): EnhancedWhoisChecker {
